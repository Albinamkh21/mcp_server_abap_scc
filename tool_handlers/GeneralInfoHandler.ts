    ...
    export class GeneralInfoHandler  {
        getTools(): ToolDefinition[] {
            return [
                ...
                {
                    name: 'getTransportObjectsByID',
                    description: 'Get transport objects by ID',
                    inputSchema: {
                        type: 'object',
                        properties: {
        ...
        async handle(toolName: string, args: any): Promise<any> {
            switch (toolName) {
                ...
                case 'getTransportObjectsByID':
                        return this.handleTransportObjects(args);    
        ...
        async handleTransportObjects(args: any) {
            try {
                if (!args?.transport_id) {
                    throw new McpError(ErrorCode.InvalidParams, 'Transport ID is required');
                }
        
                const transportId = encodeURIComponent(args.transport_id.toUpperCase());
                
                // ВАЖНО: Убираем /objects. Запрашиваем сам ресурс транспорта.
                // ADT вернет XML со структурой: Request -> Tasks -> Objects
                const transportUrl = `/sap/bc/adt/cts/transportrequests/${transportId}`;
        
                // Используем GET, так как мы читаем ресурс (аналогично чтению файла)
                const response = await makeAdtRequest(transportUrl, 'GET',30000);
        
                // Используем специальный трансформер, который умеет "нырять" внутрь задач
                return return_response(response, transformTransportRequest);
